...
if (homeSlider) {
    ...
    // Slider Numbers
    var slidecount = $('.slide-navigation__txt li').length;
    $('#totalcount').html('/' + slidecount)
    for (i = 1; i <= slidecount; i++) {
        $('#currentCount').append('<span> 0' + i + '</span>')
    }
    $('#currentCount span').eq(0).addClass('active');
    ...
    function animation_in(slide) {
        ...
        var title = slide.find('.button');
        var subtitle = $(slide).find('h2');
        var text = $(slide).find('p');
        var button = $(slide).find('button');
        var image = $(slide).find('img');

        TweenMax.fromTo(title, 1, { autoAlpha: 0, x: 100 }, { autoAlpha: 1, x: 0, ease: Power2.easeOut });
    ...
    function animation_out(slide) {
        ...
        TweenMax.to(title, 1, { autoAlpha: 0, x: 0 });
    ...
    $('.slide-navigation__txt span').on('click', function () {
        ...
        $('#currentCount span').removeClass('active');
        $('#currentCount span').eq(order - 1).addClass('active')

        // $('#currentCount span').index(order).addClass('active')
